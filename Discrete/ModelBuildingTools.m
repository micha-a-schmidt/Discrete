...
$MBDebug=False;
$MBDebug::usage="$MBDebug can be used to print debug messages. The default is False.";
...
ClearAll[MBloadGroupDefinedInGAP];
MBloadGroupDefinedInGAP::usage="MBloadGroupDefinedInGAP[pname,fname,Options] loads the group pname defined in the gap file fname with the help of GAP";
SyntaxInformation[MBloadGroupDefinedInGAP]= {"ArgumentsPattern" -> {_,_,_.}};
Options[MBloadGroupDefinedInGAP]=Options[MBloadGAPGroup];
...
ClearAll[MBgetFlavonSuperPotential];
MBgetFlavonSuperPotential::usage="MBgetFlavonSuperPotential[Group, Reps, Order, Head] returns the summed up invariants formed by Reps up to order Order using the CG coefficients of Group. The prefix of the couplings is Head.";
SyntaxInformation[MBgetFlavonSuperPotential]= {"ArgumentsPattern" -> {_,_,_,_}};
...
MBgetKroneckerProduct[pGroup_,p_,q_]:=Block[{},
	If[pGroup["KroneckerProductCalculated"][p,q]=!=True,
		pGroup["KroneckerProduct"][p,q]=generateKroneckerProduct[pGroup,p,q];
		pGroup["KroneckerProductCalculated"][p,q]=True;
		];
...
MBloadGeneratedGAPGroup[pList_,pOptions:OptionsPattern[]]:=Block[{INPUT},
  INPUT=MBconvertMathematicaToGAP[pList];
  If[$MBDebug,Print["\nINPUT:\n"]; Print[INPUT]];
  Return[MBloadGAPGroup[StringJoin["Group(",INPUT,")"],pOptions]];
  ];

MBloadGroupDefinedInGAP[pName_,fname_,pOptions:OptionsPattern[]]:=Module[{Group},
			Block[{f,p,q,OUTPUT},
				Group["Name"]=pName;
                If[$MBVerbose,Print["starting GAP generating "<>Group["Name"]<>"..."]];
		OUTPUT=RunProcess[{$MBGAP,"-b","-q"},All,"Read(\""<>fname<>"\");;Read(\""<>$MBPATH<>"extractGroupData.gap\");;\nMBgetGroupInfo(" <> Group["Name"]<>");;quit;;"];
                If[$MBDebug,Print["\nOUTPUT:\n"<>OUTPUT["StandardOutput"]<>"\n"]];
                If[$MBVerbose,Print["...finished."]];
			    f=StringToStream[readGAP[OUTPUT["StandardOutput"]]];
...
MBloadGAPGroup[pName_,OptionsPattern[]]:=Module[{Group},
			Block[{f,p,q,OUTPUT},
				Group["Name"]=pName;
                If[$MBVerbose,Print["starting GAP generating "<>Group["Name"]<>"..."]];
		OUTPUT=RunProcess[{$MBGAP,"-b","-q"},All,"Read(\""<>$MBPATH<>"extractGroupData.gap\");;MBgetGroupInfo(" <> Group["Name"]<>");;quit;;"];
                If[$MBDebug,Print["\nOUTPUT:\n"<>OUTPUT["StandardOutput"]<>"\n"]];
                If[$MBVerbose,Print["...finished."]];
			    f=StringToStream[readGAP[OUTPUT["StandardOutput"]]];
			    Group["StructureDescription"]=Read[f,Expression];
...
MBloadGroupManually[pName_,pRepMatrices_,OptionsPattern[]]:=Module[{Group},
			Block[{ltemp,lpos},
			    ...
			    If[$MBVerbose,Print["Number of irreps: ",Group["NumberOfReps"],"\n"]];
			    If[$MBVerbose,Print["Dimensions of irreps:\n",TableForm[{Range[Group["NumberOfReps"]],Group["DimensionsOfReps"]}],"\n"]];
			    ltemp=Sort[Map[{Length[#],First[#]}&,Gather[Simplify[Map[Map[Tr,#]&,Transpose[Group["RepMatrices"]]]]]]];
...
MBgetFlavonSuperPotential[pGroup_,pReps_, pOrder_, pHead_:"l", pAssumptions___] :=
  Block[{lInvariants,li,lres,ltemp},
  	If[pOrder>1,
	 lInvariants = MBgetCovariants[pGroup,pReps,pOrder-1,pAssumptions];
	 If[$MBVerbose,Print[pOrder]];
	 ...
	 ,lInvariants = {pReps};
	     ...
	     lres = 0;
         For[li = 1, li <= pOrder, li++,
             ltemp=MBgetSuperPotentialFromList[Flatten[lInvariants[[li,1]]], pHead, li];
             lres += ltemp;
         ];
         Return[lres];
	];

MBgetSuperPotentialFromList[pList_, pHead_, pEnd_] := 
  Block[{lres, li, lList, lElem, lElemCC},
   lList = pList;
   lres = 0;
   li = 1;
   While[Length[lList] > 0, lElem = First[lList];
    lList = Delete[lList, 1];
    $Assumptions=$Assumptions && Element[ToExpression[StringJoin[ToString[pHead],ToString[pEnd],"n",ToString[li]]],Reals]; 
    lres += Simplify[lElem*ToExpression[StringJoin[ToString[pHead],ToString[pEnd],"n",ToString[li]]]];
    li++;];
   Return[lres];];
...
MBconvertMathematicaToGAP[pA_Plus] := 
  replBrackets[StringRiffle[Map[MBconvertMathematicaToGAP, Apply[List,pA]],"+"]];
(*MBconvertMathematicaToGAP[pA_Times] := 
 replBrackets[StringRiffle[Map[MBconvertMathematicaToGAP, Apply[List,pA]],"*"]];*)
MBconvertMathematicaToGAP[pA_] := Block[{larg, labs,res}, 
(*AbsToString[A_] := StringReplace[{RegularExpression["Sqrt\\[(\\d+)\\]"]->"ER($1)"}][ToString[A]];*)
   larg = Arg[pA];
   labs = Abs[pA];
   If[larg == 0, Return[AbsToString[labs]], 
    If[larg == \[Pi], Return[AbsToString[-labs]],
	larg=larg/(2 \[Pi]); (* work in multiples of 2 \pi *)
	If[larg<0,larg=larg+1]; (* set argument to be between 0 and 2 *)
        res = StringJoin["E(",ToString[Denominator[larg]],")"];
	If[Numerator[larg]>1,res=StringJoin[res,"^",ToString[Numerator[larg]]]];
      If[labs==1,Return[res],Return[StringJoin[AbsToString[labs], "*", res]]];
	];
       ];
]; 
   
AbsToString[A_] := StringReplace[{"Sqrt"->"ER"}][ToString[CForm[A]]];
...
readGAP[inp0_]:=Block[{str,inp},
  spos=StringPosition[inp0,"START OUTPUT"];
  inp=StringDrop[inp0,spos[[1,2]]+1];
...
getCNormalized[sr_,sp_,sq_,Amat_]:=Block[{li,lj,lk,m,n,ll},
	{li,lj,lk}=pickTuple[sr,sp,sq,Amat];
	Return[
	Table[Flatten[Table[Table[Amat[[ll,lk]][[mIndex[sq,m,n],mIndex[sq,li,lj]]],{n,sq}],{m,sp}]],{ll,sr}]/Sqrt[Amat[[lk,lk]][[mIndex[sq,li,lj],mIndex[sq,li,lj]]]]]
];
...
If[lmulti==0,Continue[]];
			    lAold=Table[Table[A[pGroup,r,p,q,l,k],{k,MBgetDimensionOfRep[pGroup,r]}],{l,MBgetDimensionOfRep[pGroup,r]}];
					lc=getCNormalized[sr,sp,sq,lAold];
					U=Append[U,lc];
					(*lUr={Table[getCNormalized[sr,sp,sq,ll,lAold],{ll,sr}]};*)
			    For[ln=2,ln<=lmulti,ln++,
			    	lAnew=Table[Table[lAold[[ll,lk]]-Transpose[{lc[[ll]]}].Conjugate[{lc[[lk]]}],{lk,sr}],{ll,sr}];
					lc=getCNormalized[sr,sp,sq,lAnew];
